<template>
  ...
  >
    ...
    <span v-if="sidebarBrand" class="sidebar-brand" :class="sidebarBrandClass">
      ...
      >
        <b-img :src="brandImage" class="img-fluid" alt="logo" />
      </fmv-avatar>
      <span v-text="$store.state.brand" />
    </span>

    <div class="sidebar-heading">{{ $t("Estudiante") }}</div>
    <fmv-sidebar-menu :menu="i18nMenu.student" class="mb-24pt" />

    <!--
    <div class="sidebar-heading">Instructor</div>
    <fmv-sidebar-menu :menu="i18nMenu.instructor" class="mb-24pt" />

    <template v-if="menu.apps.length">
      <div class="sidebar-heading">{{ $t("Applications") }}</div>
      <fmv-sidebar-menu :menu="i18nMenu.apps" class="mb-24pt" />
    </template>

    <div class="sidebar-heading">{{ $t("Components") }}</div>
    <fmv-sidebar-menu :menu="i18nMenu.components" class="mb-24pt" />

    <div class="sidebar-heading">{{ $t("Documentation") }}</div>
    <fmv-sidebar-menu :menu="i18nMenu.documentation" class="mb-24pt" />
    -->
  </fmv-sidebar>
</template>
